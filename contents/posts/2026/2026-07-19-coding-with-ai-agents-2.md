...
それでも GPT 系の token の溶け方は尋常でない印象だ。週末に使うくらいでこんなに溶けるかね。

ここまでで散々 Claude との音楽性の違いに触れているが、仕事では OpenAI Codex/Claude Code の二刀流で進めている。
OpenAI のエンプラの契約プランには現状他社の Premium seat に相当するものがないため、ゴリゴリ使うと超過分の従量課金でドバドバ credit が消費される。これはかなり精神を蝕む。
なので探索的作業以外のコーディングの主力は [Claude Team](https://support.claude.com/en/articles/9266767-what-is-the-team-plan) の Premium Seat に流すことで token の従量課金から逃れている。

ここ 1 ヶ月ほど、仕事では [ChatGPT の Workspace agents](https://openai.com/index/introducing-workspace-agents-in-chatgpt/) を起点として GitHub Issue/PR を作る AI ワークフローを構築・運用している。
Workspace agent が毎時起動、 Slack に飛んできたサービスのエラー情報からコードをに基づいて原因と修正案を示すものだ。
内容を Agent の memory に記憶し、対話者であるヒトとエラーの対応方法を議論し、ヒトが agent との議論を経て最終的に agent  に起票を指示する。ここまでが ChatGPT Workflow agents の仕事だ。
GitHub Issue が起票されてからは、 Claude Code の Routine で PR 作成・レビューコメントに対応する。
これで基本的に Issue の内容で PR を作成してくれるが、ちょくちょく独断やレビューコメントの誘導で Issue 外の仕事もしてしまうのが難点。
レビューと最終的な merge 判断はヒトに頼りっきりだが、 GitHub Copilot code review と OpenAI Codex で PR のレビューを補助している。
...
多分課金額が明らかになっても相当高い額じゃないと利用を止めることはないだろう。
ヒトのジュニアエンジニアより安く、それよりも良い働きをするからだ。

現時点でもボトルネックがヒトである点をあぶり出されてる問題はあるけど、今後第三者的な AI のレビュワが Issue や PR に意見できるようにすれば、さらに良くなりそうだなと考えている。
ただそれでも最終的に自動起票・自動 PR したいと考えると、まだ少し距離感は感じる。
